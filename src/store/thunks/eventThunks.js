...
export const unsubscribeEventWithP24 = createAsyncThunk("order/cancelP24Subscription",
    async (eventData) => {        
        const response = await api.post("/order/cancelP24Subscription", {
...
export const createRaiseComplaint = createAsyncThunk(
    ...
    async (complaint, { rejectWithValue }) => {
        complaint = {
            ...complaint,
            flyer: complaint?.attachment,
            eventLocations: complaint?.location,
            attachment: undefined,
            location: undefined,
        };
        const response = await api.post("/event", complaint, {
