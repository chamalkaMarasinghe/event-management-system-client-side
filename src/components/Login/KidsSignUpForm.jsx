...
const KidsSignUpForm = ({ handlers, variant }) => {
  ...
  const authPopupContentMap = {
    [AUTH_POPUP_VARIANTS.REGISTER]: {
      title: languageTranslatorUtil(language,"sign_up_page","pageTitle"),
      subtitle: languageTranslatorUtil(language,"sign_up_page","formTitle"),
      footerText: languageTranslatorUtil(language,"sign_up_page","alreadyHaveAccountPrompt"),
      footerAction: languageTranslatorUtil(language,"sign_up_page","signInLinkText"),
      ...
      formConfig: {
        ...
        showEmail: true,
        buttonText: languageTranslatorUtil(language,"sign_up_page","signUpButton"),
        layout: "grid-cols-2",
      },
    },
    [AUTH_POPUP_VARIANTS.SIGNIN]: {
      title: languageTranslatorUtil(language,"sign_in_page","signInButton"),
      subtitle:  languageTranslatorUtil(language,"ms_values","signInAccount"),
      footerText: languageTranslatorUtil(language,"sign_in_page","noAccountPrompt"),
      footerAction: languageTranslatorUtil(language,"sign_in_page","signUpLinkText"),
      ...
      formConfig: {
        ...
        showEmail: true,
        buttonText: languageTranslatorUtil(language,"sign_in_page","signInButton"),
        layout: "grid-cols-1",
      },
    },
    [AUTH_POPUP_VARIANTS.FORGOT_PASSWORD]: {
      title: languageTranslatorUtil(language,"ms_values","resetPassword"),
      subtitle:languageTranslatorUtil(language,"ms_values","resetPasswordText"),
      footerText: languageTranslatorUtil(language,"reset_password_page","backToSignInText"),
      footerAction: languageTranslatorUtil(language,"sign_in_page","signInButton"),
      footerActionHandler: "switchToSignIn",
      formConfig: {
        showFirstName: false,
        showLastName: false,
        ...
        showConfirmPassword: false,
        showEmail: true,
        buttonText:languageTranslatorUtil(language,"ms_values","requestLink"),
        layout: "grid-cols-1",
      },
    },
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        switch (variant) {
          case AUTH_POPUP_VARIANTS.SIGNIN:
            ...
            if (result?.success) {
              ...
              showToast("success", languageTranslatorUtil(language,"toastMessages","staticSuccessMsgs","signInSuccess"));
          ...
          case AUTH_POPUP_VARIANTS.FORGOT_PASSWORD:
            ...
            if (result?.success) {
              showToast("success", languageTranslatorUtil(language,"toastMessages","staticSuccessMsgs","pwResetLinkSent"));
            } else {
              showToast(
                  "error",
                  result?.error?.message || languageTranslatorUtil(language,"toastMessages","staticErrors","pwResetFailure")
              );
            }
            dispatch(closeSignInModal());
            break;
          ...
          case AUTH_POPUP_VARIANTS.RESET_PASSWORD:
            ...
            if (result?.success) {
              ...
              if (result?.success) {
                showToast("success", languageTranslatorUtil(language,"toastMessages","staticSuccessMsgs","pwUpdateSuccess"));
                navigate("/", { replace: true });
                handlers["switchToSignIn"]();
              } else {
                showToast(
                    "error",
                    result?.error?.message || languageTranslatorUtil(language,"toastMessages","staticErrors","pwUpdateFailure")
                );
              }
            } else {
              showToast("error", languageTranslatorUtil(language,"toastMessages","staticErrors","tokenValidationFailure"));
            }
            break;
        ...
        showToast("error", error.message || languageTranslatorUtil(language,"toastMessages","staticErrors","errorOccurred"));
      }
    } else {
      ...
      <div className="w-full h-[67px] flex justify-center items-center flex-col gap-2">
        ...
        {variant === AUTH_POPUP_VARIANTS.REGISTER && (
            <span className="font-roboto font-normal text-[11px] leading-[18px] align-middle tracking-[0%] text-[#5C5F6A]">
              {languageTranslatorUtil(language,"ms_values","allFieldsRequired")}
            </span>
        )}
      </div>

      {/* Form Container with flex-1 to take remaining space */}
      <form className="flex-1 flex flex-col justify-between pt-2 sm:pt-0">
        {/* Form Fields Section */}
        <div className="flex-1 flex flex-col justify-start">
          ...
          >
            ...
            {content.formConfig.showPassword && (
              <CustomInput
                placeholder={languageTranslatorUtil(language,"sign_up_page","passwordLabel")}
                ...
                onChange={(e) =>{
                                    ...
                                    if(formData.confirmPassword && formData.confirmPassword !== e.target.value)
                                        errors.confirmPassword =  languageTranslatorUtil(language,"errorMessages","PASSWORD_MISMATCH")
                            }}
            ...
            {content.formConfig.showConfirmPassword && (
              <CustomInput
                placeholder={languageTranslatorUtil(language,"sign_up_page","confirmPasswordLabel")}
          ...
          {variant === AUTH_POPUP_VARIANTS.SIGNIN && (
            <div className="flex items-center justify-between mt-4">
              <div className="flex items-center gap-2">
                {/* <div className="flex items-center justify-center relative">
                  <CustomInput
                    ...
                    label={languageTranslatorUtil(language,"sign_in_page","rememberMe")}
              ...
              <div>
                ...
                >
                  {languageTranslatorUtil(language,"sign_in_page","forgotPassword")}
                </Link> */}
              </div>
            </div>
          )}
              ...
              <CustomInput
                ...
                label={
                  <span className="font-roboto text-[14px] font-normal">
                    <span className="text-[#334155]">{languageTranslatorUtil(language,"ms_values","agree")} </span>
                    <span className="text-[#F65F18]">{languageTranslatorUtil(language,"ms_values","privacyPolicy")}</span>
                  </span>
                }
