...
const AllOrders = ({
  ...
  const handleReviewSubmit = async (reviewData) => {
    try {
      console.log("Submitting review for event:", selectedOrder._id, reviewData);
      ...
      if (result?.success) {
        showToast("success", languageTranslatorUtil(language,"toastMessages","staticSuccessMsgs","reviewSubmitSuccess"));
        setIsReviewModalOpen(false);
        setSelectedOrder(null);
        // Refresh the table data to show updated information
        setRefreshTrigger(prev => prev + 1);
      } else {
        showToast("error", result?.error || languageTranslatorUtil(language,"toastMessages","staticErrors","failedSubmitReview"));
      }
    } catch (error) {
