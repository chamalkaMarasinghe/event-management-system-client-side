...
window.addEventListener("auth-state-changed", () => {
  ...
  console.log(
    `Canceling ${pendingRequests.length} pending requests due to auth state change`
  );
  ...
  async (error) => {
    ...
    if (error.response?.status === 401) {
      ...
      if (!isAuthEndpoint) {
        ...
        error.message = languageTranslatorUtil(localStorage.getItem("language"),"toastMessages","staticErrors","authenticationNeeded"); 
        error.code = "401";
