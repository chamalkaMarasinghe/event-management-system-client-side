...
// Validate email
const validateEmail = (email,errorMessages) => {
...
function validatePhone(phoneNumber,errorMessages) {
...
const validateEmailOrPhone = (input,errorMessages) => {
  if (!input) {
    return errorMessages.REQUIRED_FIELD;
  } else if (validateEmail(input,errorMessages) === "") {
    // valid email
    return "";
  } else if (validatePhone(input,errorMessages) === "") {
    // valid phone number
    return "";
  }
...
// Validate password
const validatePassword = (password,errorMessages) => {
...
const isValidNumber = (input,errorMessages) => {
...
export const validateField = (name, type, value, formData, requiredFields,language) => {

  const errorMessages = Object.fromEntries(
  Object.entries(stringTemplates.errorMessages).map(([key, item]) => [
    key,
    item[language]
  ])
);

  // console.log(name, type, value, formData, requiredFields);
  ...
  switch (fieldType) {
    ...
    case COMMON_FIELD_TYPES.NUMBER_FIELD:
      if (!value && value !== false && requiredFields[name])
        error = errorMessages.REQUIRED_FIELD;
      else if (value) error = isValidNumber(value,errorMessages);
      break;
    ...
    case COMMON_FIELD_TYPES.EMAIL_FIELD:
      if (!value && requiredFields[name]) error = errorMessages.REQUIRED_FIELD;
      else if (value) error = validateEmail(value,errorMessages);
      break;

    case COMMON_FIELD_TYPES.PHONE:
      if (!value && requiredFields[name]) error = errorMessages.REQUIRED_FIELD;
      else if (value) error = validatePhone(value,errorMessages);
      break;

    case COMMON_FIELD_TYPES.PASSWORD_FIELD:
      if (!value && requiredFields[name]) error = errorMessages.REQUIRED_FIELD;
      else if (value) error = validatePassword(value,errorMessages);
      break;
    ...
    case COMMON_FIELD_TYPES.EMAIL_OR_PHONE:
      if (!value && requiredFields[name]) {
        ...
        error = validateEmailOrPhone(value,errorMessages);
      }
      break;
