...
function detectChanges(before, after) {
  ...
  for (const prop of simpleProps) {
    if (before[prop] !== after[prop]) {
      console.log(
        `Change detected in ${prop}: ${before[prop]} → ${after[prop]}`
      );
      return true;
    }
  }
  ...
  if (
    ...
    console.log("Change detected in professional details");
    return true;
  }
