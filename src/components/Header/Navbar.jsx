...
const Navbar = () => {
  ...
  const NAV_LINKS = [
    { path: "/about-us", label: languageTranslatorUtil(language,"header_before_login","aboutUs") },
    { path: "/events", label: languageTranslatorUtil(language,"header_before_login","events") },
    { path: "/contact-us", label: languageTranslatorUtil(language,"header_before_login","contactUs") },
  ];

  // Profile dropdown links configuration
  const getProfileLinks = (isMobile = false) => {
    let links = [
      // { path: "/user-profile", label: languageTranslatorUtil(language,"header_after_login","myProfile") },
      { path: "/", label: languageTranslatorUtil(language,"header_after_login","logout") },
    ];
    ...
    if (isMobile) {
      links = [
        // { path: "/user-profile", label: languageTranslatorUtil(language,"header_after_login","myProfile") },
        // { path: "/my-wishlists", label: languageTranslatorUtil(language,"wish_list_page","title") },
        //TO-DO : missing my chat value
        // { path: "/chat", label: languageTranslatorUtil(language,"ms_values","myChat"), isChat: true },

        { path: "/signin", label: languageTranslatorUtil(language,"header_after_login","logout") },
      ];
    }

    const roleSpecificLink =
      userRole === "client"
        ? { path: "/all-orders", label: languageTranslatorUtil(language,"header_after_login","myOrders") }
                //TO-DO : missing my Events value
        : { path: "/my-events", label: languageTranslatorUtil(language,"ms_values","myEvents") };
  ...
  const buttonProps = {
    login: {
      ...
      buttonText: languageTranslatorUtil(language,"sign_in_page","signInButton") + "/" + languageTranslatorUtil(language,"sign_in_page","signUpLinkText"),
      bgColor: "bg-[#F3F4F5]",
  ...
  return (
    <nav className="h-[67px] w-full inset-0 z-[99] fixed bg-[#FFFEFC] backdrop-blur-[64px] shadow-[0px_4px_64px_0px_rgba(0,0,0,0.08)]">
      <div className="relative w-full h-full px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 2xl:px-24 mx-auto">
        <div className="relative flex items-center justify-between h-full">
          {/* Logo */}
          <div className="flex-shrink-0">
            <Link to="/" className="relative z-50 flex items-center mr-5">
              {/*<img*/}
              {/*  src={Logo}*/}
              {/*  alt="Kidsplan Logo"*/}
          ...
          <button
            ...
            className="lg:hidden relative z-50 text-xl sm:text-2xl text-gray-800 focus:outline-none p-2"
          >
          ...
          <div className="hidden lg:flex items-center w-full">
            {/* <div className="hidden custom-w-br-1180:block flex-1"></div> */}

            {/* Navigation links */}
            {/* {isAuthenticated && ( */}
            {/* <div className="flex items-center gap-4 xl:gap-6 custom-w-br-1180:flex-shrink-0 ml-auto mr-6 custom-w-br-1180:ml-0 custom-w-br-1180:mr-0 bg-red-600"> */}
            <div className={
            ...
            <div className={twMerge(isAuthenticated ? "flex items-center gap-1 flex-shrink-0 custom-w-br-1180:ml-6" : "flex items-center gap-1 flex-shrink-0 custom-w-br-1366:ml-6")}>
                      ...
                      >
                        {getProfileLinks(false).map(({ path, label }) => (
                          <Link
                            ...
                            onClick={() => {
                              setIsDropdownOpen(false);
                              if (label === "Logout" || label === "Wyloguj się") {
                                dispatch(logout());
                              }
                            }}
                          >
              ...
              <CustomButton
                  {...buttonProps.addBusiness}
                  width="w-auto"
                  className={`${buttonProps.addBusiness.className} px-4 xl:px-6`}
                  onClick={() => {
        ...
        >
          ...
          <div className="flex flex-col gap-8 w-full mb-8 text-left">
            {NAV_LINKS.map(({ path, label }) => (
              <Link
                key={path}
                to={path}
              ...
              <>
                ...
                <div ref={mobileProfileCardRef} className={STYLES.link}>
                  ...
                  >
                    ...
                    {/*/>*/}
                    <div className="w-8 h-8 rounded-full object-cover mr-3">
                      <ProfileImage
                        profilePicture={user?.profilePicture}
                        firstName={user?.firstName || ""}
                        lastName={user?.lastName || ""}
                  ...
                  {/* ToDo:Missing "Me" Value */}
                    <span>{languageTranslatorUtil(language,"ms_values","me")}</span>
                    <div className="ml-2">
                      {isMobileProfileDropdownOpen ? (
          ...
          {/* Mobile Buttons */}
          <div className="w-full max-w-xs sm:max-w-md flex flex-col gap-4 mt-auto">
            {!isAuthenticated && (
              <CustomButton
                {...buttonProps.login}
                width="w-auto"
